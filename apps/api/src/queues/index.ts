...
import { QUEUE_NAMES, type ScanJobData, type CveJobData, type ExploitJobData } from '@secscan/shared';
...
export const cveQueue = new Queue<CveJobData>(QUEUE_NAMES.CVE_SCAN, {
  connection: redisOptions,
  defaultJobOptions: {
    removeOnComplete: { count: 500 },
    removeOnFail: { count: 200 },
...
export const cveQueueEvents = new QueueEvents(QUEUE_NAMES.CVE_SCAN, { connection: redisOptions });
...
export async function getQueueStats(): Promise<Array<{ name: string; waiting: number; active: number; completed: number; failed: number; delayed: number }>> {
  const [scan, cve, exploit] = await Promise.all([
    scanQueue.getJobCounts('waiting', 'active', 'completed', 'failed', 'delayed'),
    cveQueue.getJobCounts('waiting', 'active', 'completed', 'failed', 'delayed'),
  ...
  return [
    ...
    toStats(QUEUE_NAMES.CVE_SCAN, cve as Record<string, number>),
