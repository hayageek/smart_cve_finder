...
export const api = {
  ...
  deleteVulnerabilitiesBulk: (ids: string[]) =>
    request<{ ok: boolean; deleted: number }>('/api/vulnerabilities/bulk', {
      method: 'DELETE',
  ...
  listExploitArtifacts: (id: string) =>
    request<{ files: string[]; reportDir: string }>(`/api/exploits/${id}/artifacts`),
